...
import { MatcherService,
         ...
         Candidate, 
         Question } from '../core/matcher.service';
...
export class FilterCandidatesComponent implements OnInit {
  ...
  constructor(
    ...
    this.matcher.filterDataReady$.subscribe(() => {
      this.initFilters();
    });
  }
  ...
  initFilters(): void {
    ...
    this.filters.forEach( filter => {
      ...
      if (filter instanceof CandidateFilterBasic) {

        fa = this.fb.array([], atLeastOneChecked);
        filter.getValues().forEach(v => {
          let checked = filter instanceof CandidateFilterBasic ? 
                        (filter.isExcluded(v) ? false : true) :
                        true;
